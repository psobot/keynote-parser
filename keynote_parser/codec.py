...
class IWACompressedChunk:
    ...
    def to_buffer(self):
        ...
        return b"".join([b"\x00" + struct.pack("<I", len(payload))[:3] + payload for payload in payloads])
...
class IWAArchiveSegment:
    ...
    def from_buffer(cls, buf, filename=None):
        ...
        for message_info in archive_info.message_infos:
            try:
                ...
                raise NotImplementedError("Don't know how to parse Protobuf message type " + str(message_info.type))
            try:
                ...
                raise ValueError("Failed to deserialize %s payload of length %d: %s" % (klass, message_info.length, e))
    ...
    def from_dict(cls, _dict):
        ...
        objects = []
        for message_info, o in zip(header.message_infos, _dict["objects"], strict=False):
    ...
    def to_buffer(self):
        ...
        for obj, message_info in zip(self.objects, self.header.message_infos, strict=False):
            try:
                ...
                raise ValueError("Failed to encode object: %s\nObject: '%s'\nMessage info: %s" % (e, repr(obj), message_info))
        return b"".join(
